...
def seed_database(db: Session) -> None:
    ...
    addr_shipping = Address(street="Str. Libertatii 9", city="Iasi", state="Iasi", postal_code="700001", country="Romania")
    ...
    # Categories
    # ------------------------------------------------------------------
    electronics = Category(name="Electronics", description="Gadgets, devices, and accessories")
    clothing = Category(name="Clothing", description="Apparel for all seasons")
    books = Category(name="Books", description="Fiction, non-fiction, and technical literature")
    ...
    # Products
    # ------------------------------------------------------------------
    laptop = Product(name="UltraBook Pro 14", description="Lightweight laptop with 14-inch display", price=3499.99, stock_quantity=15, category_id=electronics.id)
    headphones = Product(name="SoundMax Wireless", description="Over-ear noise-cancelling headphones", price=449.99, stock_quantity=40, category_id=electronics.id)
    tshirt = Product(name="Classic Cotton T-Shirt", description="100% cotton unisex t-shirt", price=59.99, stock_quantity=200, category_id=clothing.id)
    jacket = Product(name="Winter Puffer Jacket", description="Warm and water-resistant jacket", price=299.99, stock_quantity=50, category_id=clothing.id)
    novel = Product(name="The Silent Algorithm", description="A sci-fi thriller about AI consciousness", price=39.99, stock_quantity=80, category_id=books.id)
    ...
    # ------------------------------------------------------------------
    db.add_all([
        ProductImage(product_id=laptop.id, image_path="/images/products/ultrabook_pro_14.jpg"),
        ProductImage(product_id=headphones.id, image_path="/images/products/soundmax_wireless.jpg"),
        ProductImage(product_id=tshirt.id, image_path="/images/products/classic_tshirt_white.jpg"),
        ProductImage(product_id=tshirt.id, image_path="/images/products/classic_tshirt_black.jpg"),
        ProductImage(product_id=jacket.id, image_path="/images/products/winter_puffer_jacket.jpg"),
        ProductImage(product_id=novel.id, image_path="/images/products/the_silent_algorithm.jpg"),
    ])
    ...
    db.add_all([attr_color_black, attr_color_white, attr_size_m, attr_size_l, attr_size_xl, attr_ram_16, attr_ram_32, attr_storage_512])
    db.flush()
    ...
    db.add_all([
        OrderItem(order_id=order1.id, product_id=tshirt.id, name=tshirt.name, quantity=3, price_per_piece=tshirt.price),
        OrderItem(order_id=order1.id, product_id=novel.id, name=novel.name, quantity=1, price_per_piece=novel.price),
        OrderItem(order_id=order2.id, product_id=headphones.id, name=headphones.name, quantity=1, price_per_piece=headphones.price),
    ])
